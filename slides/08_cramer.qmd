...
# プロット
plt.figure(figsize=(8, 4))
plt.bar(x, pmf, width=1.0, tick_label=x)
plt.title(f'PMF of binomial distribution with n={n}, p={p}')
plt.xticks(np.arange(0, n+1, n//10))
plt.xlabel('$k$')
plt.ylabel('$\\Pr(X=k)$')
...
\end{align*}
の性質を改めて考えよう。
期待値が発散する場合は $+\infty$ に値を取るとみなして $K_X\colon \mathbb{R}\to(-\infty,\,+\infty]$ と考えることにする。
...
また、キュムラント母関数の簡単な性質として以下が成り立つ(今回は使わない)。
...
# キュムラント母関数 K(t) = log(1 - p + p e^t)
def K(t):
    return np.log(1 - p + p * np.exp(t))

# ルジャンドル変換 K*(x)
# K*(x) = x log(x/p) + (1-x) log((1-x)/(1-p))   (0 < x < 1)
def K_star(x):
    return x * np.log(x / p) + (1 - x) * np.log((1 - x) / (1 - p))

# t, x の範囲を取る
t = np.linspace(-4, 4, 400)
# 端点 0,1 は log が発散するので少しだけ内側を取る
...
axes[0].set_title(f"K(t) = log(1 - p + p e^t),  p={p}")
axes[0].grid(True)

# ---- 右：ルジャンドル変換 K*(x) ----
axes[1].plot(x, K_star(x))
axes[1].set_xlabel("x")
axes[1].set_ylabel("K*(x)")
axes[1].set_title("Legendre transform K*(x)")
axes[1].set_xlim(0, 1)
axes[1].grid(True)
