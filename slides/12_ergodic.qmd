...
//| fig-width: 7
//| fig-height: 2
//| fig-cap: 既約なので定常分布が唯一(一様分布)だが、周期的で定常分布に収束しない
...
\end{align*}
を証明すれば十分である。
<!-- あらかじめ $S$ の元を $\mathrm{gcd}(S)$ で割っておくことで、$\mathrm{gcd}(S)=1$ と仮定してよい。-->

任意の $S\subseteq\mathbb{Z}_{\ge 0}$ と $d\in\mathbb{Z}_{\ge 0}$ について
\begin{align*}
\mathrm{gcd}(S) = d&\iff
\left\{\sum_{i=1}^k c_i s_i \mid k\in\mathbb{Z}_{\ge 0},  c_i\in\mathbb{Z}, s_i\in S\quad\forall i\in\{1,\dotsc,k\}\right\} = d\cdot \mathbb{Z}
\end{align*}
が成り立つ。よって、ある $c_i\in\mathbb{Z}$, $s_i\in S$ が存在して、
$\sum_{i=1}^k c_i s_i = 1$
である。
$s_1 = \min_i s_i$ と仮定すると
\begin{align*}
\sum_{i=2}^k c_i s_i = 1 \bmod s_1
\end{align*}
なので、$a_i = c_i \bmod s_i$ を満たす $a_i\in\{0,1,\dotsc,s_1-1\}$ を用いて
$\sum_{i=2}^k a_i s_i = 1 \bmod s_1$
とできる。
:::
