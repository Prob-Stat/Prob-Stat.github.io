...
ある $t>0$ について、 $M_X(t)$ が存在すると仮定すると、任意の $x\in(0,t)$ について
\begin{align*}
...
\end{align*}
なので、$M_X(s)$ も存在する。
同様に、ある $t<0$ について、 $M_X(t)$ が存在すると仮定すると、任意の $x\in(t,0)$ について $M_X(s)$ も存在する。
...
証明はしないが、キュムラント母関数 $K_X(t)$ は $\mathrm{dom}(K_X)$ の**内点で微分可能であり、無限和や積分を取る前に微分しても構わない**。
